...
sub median_neighborhood {
	...
	if ($fluor eq "Cy3") {$adjbsi=3; $flags=4;}
	...
	my $upperleft; my $upperright; my $lowerleft; my $lowerright;
   	if ($array_type eq "4x44k") {
    	...
    	else {die "Array type is not 4x44k or 8x15k or 8x60k for median_neighborhood.pm"}
		...
		for (my $col=1; $col<=$topcol; $col++) {
			...
			for (my $block_col = $left_col; $block_col <= $right_col; $block_col++) {
				for (my $block_row = $top_row; $block_row <= $bottom_row; $block_row++) {
					if($$data_matrix_Aref[$block_col][$block_row][$adjbsi]ne"NA" && $$data_matrix_Aref[$block_col][$block_row][$flags]>-100 && ($$data_matrix_Aref[$block_col][$block_row][1]=~"dBr" || $$data_matrix_Aref[$block_col][$block_row][1]=~"Ctrl")) {
			...
			if ($$data_matrix_Aref[$col][$row][$flags]>-100 && $fluor eq "Cy3" && ($$data_matrix_Aref[$col][$row][1]=~"dBr" || $$data_matrix_Aref[$col][$row][1]=~"Ctrl")) {
				$$data_matrix_Aref[$col][$row][$nextcell+1] = $$data_matrix_Aref[$col][$row][$adjbsi]/$$data_matrix_Aref[$col][$row][$nextcell]*$chip_median;
			}
			elsif ($$data_matrix_Aref[$col][$row][$flags]>-100 && $fluor eq "Alexa488" && $$data_matrix_Aref[$col][$row][$lastcell] ne "NA") {
				$$data_matrix_Aref[$col][$row][$nextcell+1] = $$data_matrix_Aref[$col][$row][$adjbsi]/$$data_matrix_Aref[$col][$row][$nextcell]*$chip_median;
			}
...
sub cy3_regression{
    ...
    else {die "Array type is not 4x44k or 8x15k or 8x60k for cy3_regression.pm"}
...
sub alexa488_normalize{
    ...
    else {die "Array type is not 4x44k or 8x15k or 8x60k for alexa488_normalize.pm"}
