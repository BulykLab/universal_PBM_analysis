...
############################################################
my $cy3_gp = 1;
my $pbm_gp = 488; #KW changed 10/26/17
############################################################
...
else {die "Cannot recognize array format as either '4x44K' or '8x15K' or '8x60K'";}
...
while (my $line = <FH2>) {
	...
	else {$data_matrix[$col][$row][5] = $line[$bsi_col];} ### Cy3 BSI
	...
	for (my $tally=0; $tally<=$#line; $tally++) {
		...
		#KW changing column header to regex any number. 10/27/17
		#if ($line[$tally] eq "F$pbm_gp Median - B$pbm_gp" || $line[$tally] eq "\"F$pbm_gp Median - B$pbm_gp\"") {$bsi_col = $tally;}
	    if ($line[$tally] =~ "[\"]?F[0-9]+ Median - B[0-9]+[\"]?") {$bsi_col = $tally;}
	}
	if ($flag_col<1) {die "Unable to find 'Flags' column in $PBM_input_file";}
	if ($adjbsi_col<1 && $bsi_col<1) {die "Unable to find column labeled 'ADJBSI' or 'F# Median - B#' in $PBM_input_file";}
    }
}
...
#	if ($data_matrix[$col][$row]>-100 && ($data_matrix[$col][$row][1]=~"dBr"||$data_matrix[$col][$row][1]=~"Ctrl")) {
#	    push (@Cy3_spots, $data_matrix[$col][$row][3]);
#	}
    }
    if ($line[0] =~ "Block") {
	$spotcheck=1;
	...
	if ($bsi_col<1 && $adjbsi_col<1) {die "Unable to find column labeled 'ADJBSI' or 'F# Median - B#' in $Cy3_input_file";}
    }
}
...
my $output2 = $output_prefix."_combinatorial.txt";
...
for (my $j=1; $j<=$last_row; $j++) {
	...
	print OUTPUT1 "\n";
	print OUTPUT3 "\n";
	if ($data_matrix[$k][$j][1]=~"dBr" && $data_matrix[$k][$j][$final_index] ne "NA") {
...
open(OUTPUT2,">$output2") || die "Couldn't open 'combinatorial' output file for $output_prefix\n";
...
for (my $tally=0; $tally<=$#sorted_spots; $tally++) {
    ...
    print OUTPUT2 "$intensity{$spot_sequence}\t$spot_sequence\n";
}
close(OUTPUT2);
