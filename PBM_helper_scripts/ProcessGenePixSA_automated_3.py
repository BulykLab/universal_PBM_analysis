...
Author: Luis Barrera
Modified by Raehoon Jeong for ERIS
"""
...
#KW change paths
PBM_suite_path = '/data/bulyk/pipelines/universal_PBM_analysis/PBM_analysis_suite/'

analysis_tools_path = '/data/bulyk/pipelines/universal_PBM_analysis/PBM_helper_scripts/'
...
arg_object.add_argument('directory', metavar = 'PBM_directory',
                        help='Path to directory with array data. FILENAMES MUST HAVE ALL PROTEIN NAMES SEPARATED BY UNDERSCORES.', type = str)
...
arg_object.add_argument('numUS', metavar = 'numUS', help='number of underscores before the first protein name.')
arg_object.add_argument('-person', metavar = 'person', default='No Person',help='Person who ran the experiment.')
arg_object.add_argument('-annotations', metavar = 'annotations', default='No Annotations', help='any relevant information about the array')
...
try:
    int(args.numUS)
    if int(args.numUS)<0:
        raise Exception("The number of underscores must be a positive integer.")
except ValueError:
    raise Exception("The number of underscores must be a positive integer.")
...
master_masliner_command = "perl "+PBM_suite_path+"masliner.pl -g1 %s -g2 %s -f1 488 -f2 488 -o %s -ll 200 -lh 40000"
...
if args.noSW is True:
    ...
    #SubmitJobsAndWait(jobs_queue)
    """
    print('Running S&W + BEEML-PBM jobs...')
    ...
    with open(args.directory + 'report.html', 'w') as f_out:
        ...
        MakeTable(f_out, protein_names, '_primary','height=120')  ## was 110
        MakeTable(f_out, protein_names, '_secondary','height=120')  ## was 110
        f_out.write('<br><br><br><br>')
        MakeTable(f_out, protein_names, '_BEEML','height=250')  ## there was no height
