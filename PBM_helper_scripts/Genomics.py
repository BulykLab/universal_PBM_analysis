...
def GenerateTempFilename(temp_dir = '/data/bulyk/pipelines/universal_PBM_analysis/PBM_tmp_files/', base_string = None):
    ...
    if base_string is None:
        ...
        return temp_dir + hashlib.md5((time.ctime() + str(random())).encode('utf-8')).hexdigest()

    else:

        return temp_dir + hashlib.md5(base_string.encode('utf-8')).hexdigest()
...
class PWM():
    ...
    def __init__(self, matrix, background = np.array([0.25, 0.25, 0.25, 0.25]), suppress_pwm = False, name = None, reverse = False, **kwargs):
        ...
        if suppress_pwm is False:
            ...
            ## Changed
            #print(self.pfm)
            self.pwm = np.array([1.0/self.background * self.pfm[:,i]  for i in range(self.pfm.shape[1])])
            self.pwm = np.vstack(self.pwm).T
    ...
    def Trim(self, threshold = 0.5):
        ...
        if left_bound is None or right_bound is None:
            prin("There are no consecutive positions that meet the IC threshold given, so no trimmed logo could be made.")
    ...
    def MakeLogo(self, filename, title = "", reverse_complement = False, num_seqs = 1000,  **kwargs):
        ...
        from io import StringIO   # Changed
        ...
        subprocess.call('weblogo -f %s -o %s -F %s -P "" -t "%s" --resolution 1000 --errorbars NO -c classic -s large --annotate %s --debug NO' \
